...
/*   Updated: 2023/03/29 16:55:05 by jgo              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	sem->start_sem = sem_open(START_SEM_NAME, O_CREAT | O_EXCL, 0644, 1);
	if (sem->start_sem = SEM_FAILED)
		return (prt_err(ERR_INIT_MUTEX, SET_ERROR));
	sem->terminate_sem = sem_open(TERMINATE_SEM_NAME, O_CREAT | O_EXCL, 0644, num_of_philo);
	if (sem->terminate_sem = SEM_FAILED)
		return (prt_err(ERR_INIT_MUTEX, SET_ERROR));
	sem->eat_cnt_sem = sem_open(EAT_CNT_SEM_NAME, O_CREAT | O_EXCL, 0644, num_of_philo);
	if (sem->eat_cnt_sem = SEM_FAILED)
		return (prt_err(ERR_INIT_MUTEX, SET_ERROR));
	sem->last_meal_sem = sem_open(LAST_MEAL_SEM_NAME, O_CREAT | O_EXCL, 0644, num_of_philo);
